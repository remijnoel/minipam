#!/bin/bash
# Build and package external UI for MiniPAM with correct base path
...
echo "🎨 Building and packaging external UI for MiniPAM with base path"
...
# Check if it's a Vite project and update base path
if [ -f "vite.config.ts" ] || [ -f "vite.config.js" ]; then
    echo "🔧 Detected Vite project, setting base path to /ui/"
    
    # Create or update vite config with base path
    ...
    if grep -q "base:" "$CONFIG_FILE"; then
        ...
        sed -i.tmp "s|base:.*,|base: '/ui/',|g" "$CONFIG_FILE"
    else
        # Add base to defineConfig
        sed -i.tmp "s|defineConfig({|defineConfig({\n  base: '/ui/',|g" "$CONFIG_FILE"
    fi
...
if command -v npm &> /dev/null; then
    ...
    echo "🔨 Building UI with base path..."
    npm run build
...
if [ -f "$DIST_DIR/index.html" ]; then
    echo "🔧 Updating index.html asset paths..."
    # Update src="/assets/ to src="/ui/assets/
    sed -i.bak 's|src="/assets/|src="/ui/assets/|g' "$DIST_DIR/index.html"
    # Update href="/assets/ to href="/ui/assets/
    sed -i.bak 's|href="/assets/|href="/ui/assets/|g' "$DIST_DIR/index.html"
    # Update any other absolute paths
    sed -i.bak 's|href="/favicon|href="/ui/favicon|g' "$DIST_DIR/index.html"
    sed -i.bak 's|src="/favicon|src="/ui/favicon|g' "$DIST_DIR/index.html"
...
if [ -d "$UI_TARGET_DIR/dist" ] && [ -f "$UI_TARGET_DIR/dist/index.html" ]; then
    echo ""
    echo "✅ UI successfully packaged with correct base path!"
    echo "📁 UI files available at: $UI_TARGET_DIR/dist/"
    echo "🌐 Will be served at /ui when the app runs"
